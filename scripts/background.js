...
async function setNextAlarm(tabs) {
  ...
  next = next.length
    ? next.reduce((t1, t2) => (t1.wakeUpTime < t2.wakeUpTime ? t1 : t2))
    : undefined;
  if (!next) return;
...
async function snoozeInBackground(item, tab) {
  ...
  var assembledTab = Object.assign(item, {
    url,
    title,
    pinned,
    startUp,
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (chrome.runtime.setUninstallURL)
    chrome.runtime.setUninstallURL("https://dozetabs.vercel.app/bye");
  if (details && details.reason && details.reason == "install")
    await new Promise((r) =>
      chrome.tabs.create(
        { url: "https://dozetabs.vercel.app/hello", active: true },
        r,
      ),
    );
  if (
