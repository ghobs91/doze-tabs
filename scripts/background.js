...
async function snoozeInBackground(item, tab) {
	...
	if(!isValid({url})) return createNotification(null, `Can't doze that :(`, 'icons/logo.svg', 'The link you are trying to snooze is invalid.', true);
	...
	if (!snoozeTime || c.disabled || dayjs().isAfter(dayjs(snoozeTime))) {
		return createNotification(null, `Can't doze that :(`, 'icons/logo.svg', 'The time you have selected is invalid.', true);
	}
	// add attributes
...
chrome.runtime.onInstalled.addListener(async details => {
	setUpExtension();
	if (chrome.runtime.setUninstallURL) chrome.runtime.setUninstallURL('https://dozetabs.vercel.app/bye');
	if (details && details.reason && details.reason == 'install') await new Promise(r => chrome.tabs.create({url: 'https://dozetabs.vercel.app/hello', active: true}, r));
