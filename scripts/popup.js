...
async function init() {
  ...
  document.querySelectorAll(".nap-room-btn, .settings").forEach((btn) =>
    btn.addEventListener("click", (el) => {
      openExtensionTab(el.target.dataset.href);
      setTimeout((_) => window.close(), 100);
    }),
  );
  document.querySelectorAll(".nap-room-btn, .settings").forEach(
    (btn) =>
      (btn.onkeyup = (e) => {
  ...
  if (!(isInEditMode || isInDupeMode) && tabs && tabs.length) {
    var todayCount = sleeping(tabs).filter(
      (t) =>
    ...
    if (todayCount > 0)
      document
        ...
        .setAttribute("data-today", todayCount);
  }
  document.getElementById("repeat").addEventListener("change", toggleRepeat);

  document.addEventListener("keyup", (e) => {
    var isOverlayOpen = document
      .querySelector(".form-overlay")
      .classList.contains("show");
    if (e.keyCode >= 49 && e.keyCode <= 58 && !isOverlayOpen) {
      var choices = document.querySelectorAll(".choice");
      var selectedChoice =
        choices && choices.length > 0 ? choices[e.keyCode - 48 - 1] : false;
      if (!selectedChoice || selectedChoice.classList.contains("disabled"))
        return;
    ...
    if (e.keyCode === 67 && !isOverlayOpen)
      document.querySelector(".custom-choice").click();
    if (e.keyCode === 67 && isOverlayOpen)
      document.querySelector(".overlay-close-btn").click();
    if (e.keyCode === 84 && !isOverlayOpen)
      document.getElementById("tab").click();
    if (e.keyCode === 87 && !isOverlayOpen)
    ...
    if (
      (isInEditMode || isInDupeMode) &&
      parent &&
      parent.closeModalOnOutsideClick
    )
      parent.closeModalOnOutsideClick(e);
  });
  ...
  if ((isInEditMode || isInDupeMode) && parent && parent.resizePopupIframe)
    parent.resizePopupIframe();
}
async function initEditMode(isDupe) {
  document.querySelector("h3").innerText = isDupe
    ? "Duplicate What?"
    : "Edit What?";
  ...
  document.getElementById("preview-favicon").src = t.tabs
    ? `../icons/${iconTheme}/${t.selection ? "selection" : "window"}.png`
    : getUrlParam("noImg")
      ? "../icons/unknown.png"
      : getFaviconUrl(t.url);
  document
    .getElementById(t.tabs ? (t.selection ? "selection" : "window") : "tab")
...
async function toggleRepeat(e) {
  ...
  Object.entries(choices).forEach(async ([name, o]) => {
    ...
    c.classList.toggle(
      "disabled",
      (!repeat && !!o.disabled) || (repeat && !!o.repeatDisabled),
    );
    c.classList.toggle(
      "always-disabled",
      (!repeat && !!o.disabled) || (repeat && !!o.repeatDisabled),
    ...
    c.querySelector(".label .text").innerText = repeat
      ? o.repeatLabel
      : o.label;
    if (name !== "startup") {
      c.querySelector(".date").innerText = repeat
        ? o.repeatTimeString
        : o.timeString;
      if (!repeat) console.log(o.label, o.time);
      c.querySelector(".time").innerText = repeat
        ? o.repeatTime
        : dayjs(o.time).format(
            `${getHourFormat(dayjs(o.time).minute() !== 0)}`,
          );
    }
    if (["weekend", "monday", "week", "month"].includes(name))
      c.querySelector("select").dispatchEvent(new Event("change"));
  });
  var custom = document.getElementById("custom");
  custom.querySelector(".label").innerText = repeat
    ? "Choose a custom interval"
    : "Choose your own time";
  document
    .querySelector(".repeat-container")
    .classList.toggle("hidden", !repeat);
}
...
async function buildTargets() {
  ...
  document
    .getElementById("tab")
    .classList.toggle("disabled", !isActiveTabValid);
  var isWindowValid =
    getBrowser() !== "safari" &&
    (validTabs.length > 1 || (validTabs.length === 1 && !isActiveTabValid));
  document
    .getElementById("window")
    .classList.toggle("disabled", !isWindowValid);
  var isSelectionValid =
    getBrowser() !== "safari" &&
    validTabs.length > 1 &&
    activeTab.highlighted &&
    validTabs.filter((t) => t.highlighted).length > 1;
  document
    .getElementById("selection")
    .classList.toggle("disabled", !isSelectionValid);

  document
    .querySelectorAll(".target")
    .forEach((t) => (t.tabIndex = t.classList.contains("disabled") ? -1 : 0));
  document.querySelectorAll(".target").forEach((t) =>
    t.addEventListener("keyup", (e) => {
      if (e.which === 13) t.click();
    }),
  );

  if (getBrowser() !== "chrome" || true)
  ...
  if (isSelectionValid) {
    ...
    document
      .querySelectorAll(".choice, .custom-choice, h3, .repeat-choice, #repeat")
      .forEach((c) => {
        c.classList.add("disabled");
        c.setAttribute("tabindex", "-1");
      });
    return (document.getElementById("preview-text").innerText =
      `Can't snooze this tab`);
  }
  await generatePreview(document.querySelector(".target.active").id);

  document.querySelectorAll(".target").forEach((t) =>
    t.addEventListener("click", async (e) => {
      if (t.classList.contains("disabled") || t.classList.contains("active"))
        return;
      document
        .querySelectorAll(".target")
...
async function generatePreview(type) {
  ...
  if (type === "tab") {
    ...
    previewIcon.onload = (_) => {
      if (previewIcon && previewIcon.height === 16 && previewIcon.width === 16)
        previewIcon.src = "../icons/unknown.png";
    };
    previewIcon.src = a.favIconUrl
      ? a.favIconUrl
      : getBrowser() === "safari"
        ? getFaviconUrl(a.url)
        : "../icons/unknown.png";
    ...
    var validTabs = allTabs.filter(
      (t) => !isDefault(t) && isValid(t) && t.highlighted,
    );
...
async function buildChoices() {
  var choices = await getChoices();
  var config = await getOptions(["popup"]);
  colorList = gradientSteps(
    "#F3B845",
    "#DF4E76",
    Math.ceil(Object.keys(choices).length / 2) + 1,
  );
  document.querySelector(".section.choices").append(
    ...Object.entries(choices).map(([name, o], i) => {
      var icon = Object.assign(document.createElement("img"), {
        src: `../icons/${iconTheme}/${name}.png`,
      });

      var selectWrapper = "";
      if (["weekend", "monday", "week", "month"].includes(name)) {
        var s =
          config.popup && config.popup[name] ? config.popup[name] : "morning";
        var morning = Object.assign(document.createElement("option"), {
          value: "morning",
          innerText: "Morning",
          selected: s === "morning",
        });
        var evening = Object.assign(document.createElement("option"), {
          value: "evening",
          innerText: "Evening",
          selected: s === "evening",
        });
        var now = Object.assign(document.createElement("option"), {
          value: "now",
        ...
        select.addEventListener("change", async (e) => {
          ...
          document.querySelector(`#${name} .time`).innerText = t.format(
            getHourFormat(t.minute() !== 0),
          );
          // resize dropdown
          var d = Object.assign(document.createElement("select"), {
            style: { visibility: "hidden", position: "fixed" },
          });
          var o = Object.assign(document.createElement("option"), {
            innerText: e.target.options[e.target.selectedIndex].text,
          });
          d.append(o);
          e.target.after(d);
      ...
      var label = wrapInDiv(
        { classList: "label" },
        wrapInDiv({ className: "text", innerText: o.label }),
        selectWrapper,
      );
      var date = wrapInDiv({ classList: "date", innerText: o.timeString });
      var time = wrapInDiv({
        classList: "time",
        innerText: dayjs(o.time).format(
          getHourFormat(dayjs(o.time).minute() !== 0),
        ),
      });
      ...
      var c = wrapInDiv(
        ...
        wrapInDiv("", icon, label),
        o.startUp ? wrapInDiv() : wrapInDiv("", date, time),
      );
      ...
      if (["weekend", "monday", "week", "month"].includes(name))
        c.addEventListener("keydown", (e) => {
          ...
          if (e.which === 38 && current > 0)
            options[current - 1].selected = true;
          if (e.which === 40 && current < options.length - 1)
            options[current + 1].selected = true;
          select.dispatchEvent(new Event("change"));
        });
      c.onclick = (e) => {
        if (!["OPTION", "SELECT"].includes(e.target.nodeName))
          snooze(o.startUp ? "startup" : o.time, c);
      };
      c.onkeyup = (e) => {
        if (e.which === 13 || e.which === 32)
          snooze(o.startUp ? "startup" : o.time, c);
  ...
  document
    .querySelectorAll(".section.choices .choice select")
...
async function buildRepeatCustomChoice() {
  ...
  dayNames
    ...
    .forEach((day) => {
      ...
      document
        .querySelector(".repeat-week-wrapper div")
        .append(wrapInDiv({ className: "day-choice" }, span));
    });
  ...
  var reset = (_) => {
    ...
    document
      .querySelectorAll(".day-choice span.active")
      .forEach((s) => s.classList.remove("active"));
    validate();
  };
  ...
  var validate = async (_) => {
    ...
    var isValid = false,
      isWeekly =
        document
          .querySelector(".repeat-interval.active")
          .getAttribute("data-type") === "weekly";
    document.querySelector(".date-display").innerText =
      `Select ${isWeekly ? "days" : "dates"}`;
    document.querySelector(".time-display").innerText = dayjs(
      time.selectedDates,
    ...
    if (isWeekly) {
      var days = Array.from(
        document.querySelectorAll(".day-choice span.active"),
      ).map((s) => parseInt(s.getAttribute("data-value")));
      if (days.length) {
        document.querySelector(".date-display").innerText =
          days.length <= 2
            ? `${days.map((d) => dayNames[d].substring(0, 3)).join(", ")} every week`
            : `${days.length} days every week`;
        isValid = true;
      }
      ...
      document.querySelector(".date-display").innerText =
        dates.length === 1
          ? `${dates.map((d) => getOrdinal(dayjs(d).format("D"))).join(", ")} of every month`
          : `${dates.length} days every month`;
    }
    document.querySelectorAll(".repeat-time-wrapper .action").forEach((a) => {
      var action = a.getAttribute("data-value"),
        actionValue = dayjs();
      if (action == "morning")
        actionValue = dayjs()
          .startOf("d")
          .add(config.morning[0], "h")
          .add(config.morning[1], "m");
      if (action == "evening")
        actionValue = dayjs()
          .startOf("d")
          .add(config.evening[0], "h")
          .add(config.evening[1], "m");
      a.classList.toggle(
        "disabled",
        dayjs(time.selectedDates).format("HHmm") === actionValue.format("HHmm"),
      );
    });
    if (isValid) {
      var data = {
        type: "custom",
        time: [
          dayjs(time.selectedDates).hour(),
          dayjs(time.selectedDates).minute(),
        ],
      };
      if (
        document
        ...
        data.weekly = Array.from(
          document.querySelectorAll(".day-choice span.active"),
        )
          .map((d) => parseInt(d.getAttribute("data-value")))
          .sort(desc);
      } else {
        data.monthly = document
          .getElementById("monthly")
          ._flatpickr.selectedDates.map((d) => dayjs(d).date())
          .sort(desc);
      }
      var wakeUpTime = await calculateNextSnoozeTime(data);
      document.getElementById("next-wakeup").innerText = formatSnoozedUntil({
        wakeUpTime,
      });
  ...
  if (document.querySelector(".repeat-time-wrapper .f-am-pm"))
    document
      .querySelector(".repeat-time-wrapper .f-am-pm")
      .addEventListener("click", validate);
  document
    .querySelector(".repeat-time-wrapper .reset-action")
  ...
  document
    .querySelector(".repeat-month-wrapper .f-days")
    .addEventListener("click", validate);
  document.querySelectorAll(".repeat-time-wrapper input").forEach((i) => {
    ...
    i.addEventListener("keyup", (e) => {
      if (e.which && (e.which === 38 || e.which === 40)) validate();
    });
  });
  document.querySelectorAll(".repeat-time-wrapper .action").forEach((a) =>
  ...
  document.getElementById("repeat").addEventListener("change", (e) => {
    if (e.target.checked) validate();
  });
  document.querySelectorAll(".repeat-interval").forEach((ri) =>
    ri.addEventListener("click", (e) => {
      if (e.target.classList.contains("active")) return;

      document
        .querySelectorAll(".repeat-interval")
        .forEach((rs) => rs.classList.remove("active"));
      e.target.classList.add("active");

      document
        .querySelectorAll(".repeat-section > div.r-section")
        .forEach((rdw) => rdw.classList.add("hidden"));
      if (e.target.getAttribute("data-type") == "weekly")
        document
          .querySelector(".repeat-week-wrapper")
          .classList.remove("hidden");
      if (e.target.getAttribute("data-type") == "monthly") {
        document
          .querySelector(".repeat-month-wrapper")
          .classList.remove("hidden");
        var m = document.querySelector(".repeat-month-wrapper"),
          days = Array.from(
            m.querySelectorAll(
              ".dayContainer .f-day:not(.f-disabled):not(.nextMonthDay)",
            ),
          );
        var bounds = days
        ...
        var func = (x, y) => {
          bounds
            .filter(
              (b) => b.left <= x && x <= b.right && b.top <= y && y <= b.bottom,
            )
            .forEach((b) => {
              date.setDate([
                ...new Set(
                  date.selectedDates
                    ...
                    .concat(`2020-03-${days[b.index].innerText}`),
                ),
              ]);
            });
          validate();
        ...
        m.addEventListener("mousedown", (e) => {
          e.preventDefault();
          m.addEventListener("mouseover", selectMultipleDates);
        });
        m.addEventListener("mouseup", (e) => {
          e.preventDefault();
          m.removeEventListener("mouseover", selectMultipleDates);
        });
...
async function buildCustomChoice() {
  ...
  var getDateTime = (_) => {
    return dayjs(
      dayjs(date.selectedDates).format("YYYY-MM-DD") +
        dayjs(time.selectedDates).format("HH:mm"),
    );
  };
  ...
  var validate = async (_) => {
    ...
    document.querySelectorAll(".time-wrapper .action").forEach((action) => {
      action.classList.toggle(
        "disabled",
        getDateTime().add(parseInt(action.getAttribute("data-value")), "m") <
          now,
      );
    });
    ...
    time.set(
      "minTime",
      getDateTime().dayOfYear() === now.dayOfYear()
        ? now.format("HH:mm")
        : null,
    );
    document.querySelector(".date-display").innerText = dayjs(
      date.selectedDates,
    ).format("ddd, D MMM");
    document.querySelector(".time-display").innerText = dayjs(
      time.selectedDates,
    ...
    document
      ...
      .classList.toggle("disabled", getDateTime() <= now);
  ...
  var icon = Object.assign(document.createElement("img"), {
    src: `../icons/${iconTheme}/custom.png`,
  });
  var label = wrapInDiv({
    classList: "label",
    innerText: "Choose your own time",
  });
  var customChoice = wrapInDiv(
    {
      ...
      onclick: (_) => {
        ...
        document
          .querySelector(".popup-checkbox input")
    ...
    wrapInDiv(
      ...
      wrapInDiv(
        "display",
        wrapInDiv("date-display"),
        wrapInDiv("time-display"),
      ),
      submitButton,
    ),
  );
  ...
  customChoice.addEventListener("mouseover", (_) =>
    customChoice.classList.add("really-focused"),
  );
  customChoice.addEventListener("mouseout", (_) =>
    customChoice.classList.remove("really-focused"),
  );
  ...
  document.querySelectorAll(".time-wrapper .action").forEach((action) =>
    action.addEventListener("click", (_) => {
      ...
      if (Math.abs(amount) > 1000) {
        ...
        if (
          dayjs(time.selectedDates).add(amount, "m").dayOfYear() !=
          dayjs(time.selectedDates).dayOfYear()
        ) {
          date.setDate(
            dayjs(date.selectedDates)
              .add(amount < 0 ? -1 : 1, "d")
              .toDate(),
          );
        }
  ...
  if (document.querySelector(".time-wrapper .f-am-pm"))
    document
      .querySelector(".time-wrapper .f-am-pm")
      .addEventListener("click", validate);

  document
    .querySelector(".time-wrapper .reset-action")
    .addEventListener("click", (_) => {
      reset();
      validate();
    });
  ...
  document
    .querySelector(".date-wrapper .f-days")
    .addEventListener("click", (e) => {
      if (e.target.classList.contains("f-day")) validate();
    });
...
async function modify(time, choice) {
  if (parent && parent.deleteTabFromDiv)
    parent.deleteTabFromDiv(getUrlParam("tabId"));
  var response = await editSnoozed(getUrlParam("tabId"), time, isInDupeMode);
  if (!response.edited && !response.duped) return;
  await displayPreviewAnimation(
    choice,
    time.format ? time.format(".HHmm") : "",
    response.duped ? "Welcome to the clone zone" : "Going back to sleep",
  );
  if (parent && parent.closePopupModal)
    setTimeout((_) => parent.closePopupModal(), closeDelay);
}

async function snooze(time, choice) {
  time = ["weekend", "monday", "week", "month"].includes(choice.id)
    ? await getTimeWithModifier(choice.id)
    : time;
  var response,
  ...
  if (document.getElementById("repeat").checked) {
    var t,
      data = { type: choice.getAttribute("data-repeat-id") };
    ...
    if (data.type === "custom") {
      var pickr = dayjs(
        document.getElementById("repeat-time")._flatpickr.selectedDates,
      );
      ...
      if (
        document
          .querySelector(".repeat-interval.active")
          .getAttribute("data-type") === "monthly"
      ) {
        data.monthly = document
          .getElementById("monthly")
          ._flatpickr.selectedDates.map((d) => dayjs(d).date())
    ...
    if ((isInEditMode || isInDupeMode) && getUrlParam("tabId")) {
      if (parent && parent.deleteTabFromDiv)
        parent.deleteTabFromDiv(getUrlParam("tabId"));
      response = await editRecurringSnoozed(
        getUrlParam("tabId"),
        data,
        isInDupeMode,
      );
      ...
      await displayPreviewAnimation(
        choice,
        time.format ? time.format(".HHmm") : "",
        response.duped ? "Duplicating..." : "Going back to sleep",
      );
      if (parent && parent.closePopupModal)
        setTimeout((_) => parent.closePopupModal(), closeDelay);
    } else {
  ...
  await chrome.runtime.sendMessage(
    Object.assign(response, { close: true, delay: closeDelay }),
  );
  await displayPreviewAnimation(
    choice,
    time.format ? time.format(".HHmm") : "",
